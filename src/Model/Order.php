...
class Order extends AbstractModel
{
     ...
     * been made, the <strong>cancelState</strong> field is returned with a value of
     * <code>NONE_REQUESTED</code>, and an empty <strong>cancelRequests</strong> array
     * is also returned.
     *
     ...
     * format, which uses the 24-hour Universal Coordinated Time (UTC) clock. <br /><br
    ...
    /**
     * This array contains a list of one or more
     * <strong>getShippingFulfillment</strong> call URIs that can be used to retrieve
     * shipping fulfillments that have been set up for the order.
     *
     * @var string[]
     */
     ...
     * packaging, addressing, and shipping the order.<br /><br /><span
     * class="tablenote"><strong>Note:</strong> Although this container is presented as
     * an array, it currently returns only one set of fulfillment specifications.
     * Additional array members will be supported in future functionality.</span>.
     *
     ...
     * the Trading API (and other legacy APIs). Both the <b>orderId</b> field and this
     * field are always returned. <br/><br/><span
     * class="tablenote"><strong>Note:</strong> In June 2019, Order IDs in REST APIs
     * transitioned to a new format. For the Trading and other legacy APIs, by using
     * version control/compatibility level, users have the option of using the older
     * legacy order ID format, or they can migrate to the new order ID format, which is
     * the same order ID format being used by REST APIs. Although users of the Trading
     * API (and other legacy APIs) can now transition to the new order ID format, this
     * <strong>legacyOrderId</strong> field will still return order IDs in the old
     * format to distinguish between the old and new order IDs.</span>.
     *
     * @var string
     */
     ...
     * <strong>OrderFulfillmentStatus</strong> type definition for more information
     * about each possible fulfillment state. For implementation help, refer to <a
    ...
    /**
     * The unique identifier of the order. Both the <b>legacyOrderId</b> field
     * (traditionally used by Trading and other legacy APIS) and this field are always
     * returned.  <br/><br/><span class="tablenote"><strong>Note:</strong> In June
     * 2019, Order IDs in REST APIs transitioned to a new format. For the Trading and
     * other legacy APIs, by using version control/compatibility level, users have the
     * option of using the older legacy order ID format, or they can migrate to the new
     * order ID format, which is the same order ID format being used by REST APIs. The
     * new format is a non-parsable string, globally unique across all eBay
     * marketplaces, and consistent for both single line item and multiple line item
     * orders. These order identifiers are automatically generated after buyer payment,
     * and unlike in the past, instead of just being known and exposed to the seller,
     * these unique order identifiers will also be known and used/referenced by the
     * buyer and eBay customer support.</span>.
     *
     * @var string
     */
    public $orderId = null;
     ...
     * refund. See the <strong>OrderPaymentStatusEnum</strong> type definition for more
     * information about each possible payment/refund state. For implementation help,
     * refer to <a
     ...
     * in the <code>fulfillmentStartInstructions.shippingStep.shipTo</code> container.
     * If the item is successfully authenticated, the authenticator will ship the item
     * to the buyer.
     *
     ...
     * Hub. A <strong>salesRecordReference</strong> number is only generated and
     * returned at the order level, and not at the order line item level.<br><br> In
     * cases where the seller does not have a Selling Manager or Selling Manager Pro
     * subscription nor access to Seller Hub, this field may not be returned.
     *
     * @var string
     */
